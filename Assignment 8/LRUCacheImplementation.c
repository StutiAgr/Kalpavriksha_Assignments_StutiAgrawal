...
int stringToInteger(char* string, int* result){
...
int hashMapSearch(int key){
    ...
    for(int i = 0; i < cacheCapacity; i++){
        ...
        if(lruCache[try].queuePtr == NULL) return -1;
...
void handleCreateCache(char* command, int* cacheCreated, int* capacity){
    ...
    if(!stringToInteger(capacityStr, capacity)){
...
void initiateLRUCache(){
    ...
    while(1){
        ...
        if(command[stringLength(command) - 1] == '\n'){
            command[stringLength(command) - 1] = '\0';
        }
        char function[20];
